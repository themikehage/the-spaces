...
export { ModelsError, type AuthModel, type ModelsErrorCode } from "./auth/resolve.ts";
...
class ModelsImpl implements MutableModels {
  ...
  async refresh(provider?: string): Promise<void> {
    ...
    await Promise.allSettled(
      Array.from(this.providers.values(), async (entry) => entry.refreshModels?.()),
    );
  }
...
export function calculateCost<TApi extends Api>(model: Model<TApi>, usage: Usage): Usage["cost"] {
  ...
  const cacheReadCost = model.cost.cacheRead ?? 0;
  const cacheWriteCost = model.cost.cacheWrite ?? 0;
  ...
  usage.cost.cacheWrite = (cacheWriteCost * shortWrite + inputCost * 2 * longWrite) / 1000000;
  usage.cost.total =
