...
function parseUsage(
  ...
  const usage = {
    ...
    cost: {
      input: ((model.cost?.input ?? 0) / 1000000) * input,
      output: ((model.cost?.output ?? 0) / 1000000) * output,
      cacheRead: ((model.cost?.cacheRead ?? 0) / 1000000) * cacheReadTokens,
      cacheWrite: ((model.cost?.cacheWrite ?? 0) / 1000000) * cacheWriteTokens,
      total: 0,
    },
  };
  usage.cost.total =
