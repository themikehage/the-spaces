...
const DEFAULT_LITERALS = {
  bodyNoMemories: "No memories found",
...
function RecallView({
  ...
  return (
    <div className="flex flex-col gap-1.5 font-mono text-[11px]">
      ...
      {memories.map((m: any) => {
        const colorClass = TYPE_COLORS[m.type] ?? "text-muted-foreground bg-surface";
        return (
          <div key={m.id} className="rounded-md border border-input/40 overflow-hidden">
            <div className="flex items-center gap-2 px-3 py-1.5 bg-card border-b border-input/20">
              ...
              <span className="text-muted-foreground text-[10px] ml-auto font-mono truncate max-w-[120px]">
                {m.id}
              </span>
            </div>
            <div className="px-3 py-2 text-muted-foreground text-[11px] whitespace-pre-wrap leading-relaxed max-h-32 overflow-y-auto">
              {m.content}
            </div>
            {m.tags && m.tags.length > 0 && (
              <div className="flex items-center gap-1 px-3 py-1.5 border-t border-input/20 bg-muted/30">
                {m.tags.map((tag: string) => (
                  <span
                    key={tag}
                    className="px-1.5 py-0.5 rounded bg-surface text-muted-foreground text-[10px]"
                  >
                    #{tag}
                  </span>
                ))}
...
function StoreView({
  ...
  const colorClass = TYPE_COLORS[type] ?? "text-muted-foreground bg-surface";
  ...
  return (
    <div className="flex flex-col gap-1.5 font-mono text-[11px]">
      ...
      {content && (
        <div className="px-3 py-2 rounded-md border border-input/40 bg-card text-muted-foreground text-[11px] whitespace-pre-wrap leading-relaxed max-h-32 overflow-y-auto">
          {content}
        </div>
      )}
      {tags.length > 0 && (
...
function ForgetView({
  ...
  return (
    <div className="font-mono text-[11px] text-muted-foreground">
      ...
      {id && <span className="ml-2 text-muted-foreground">{id}</span>}
    </div>
  );
}

export function MemoryResult({ mode, args, details, l = DEFAULT_LITERALS }: Props) {
  switch (mode) {
    case "recall":
