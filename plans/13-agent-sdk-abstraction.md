...
const runtime = createAgentRuntime({
  username, sessionId, projectId, agentId, toolProfile: "user-session"
});
```
...
```ts
class SpacesRunner {
  constructor(agent: SpacesAgent, options?: {
    workspaceDir?: string;
    sessionId?: string;
  });
...
export const bash = new FunctionTool({
  ...
  execute: async (params) => { /* sandbox execution */ },
});
```
...
class SpacesAgent {
  ...
  async createRuntime(username: string, sessionId: string): Promise<AgentRuntimeInstance> {
    ...
    const workspaceDir = this.config.workspaceDir 
      ?? getAgentWorkspaceDir(username, this.config.name);
