...
- Router REST de Configuración por Entidad `/api/config` para consulta, actualización (`PUT`) y resolución en cascada (`GET /:entityType/:entityId/resolved`), más endpoint de conveniencia `GET /api/sessions/:id/config`.
