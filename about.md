...
- **`apps/mobile`**: aplicación móvil multiplataforma en Flutter (iOS/Android) con Riverpod, GoRouter, Dio, WebSocket con reconexión automática y contratos compartidos sincronizados. Incluye autenticación con guard reactivo, Dashboard operativo, lista paginada de sesiones con filtros y swipe-to-delete, Chat en tiempo real con streaming token-a-token, renderizado de Markdown, tarjetas de herramientas (Tool Calls) colapsables, selector de modelos, gestión de proyectos y agentes con inspector de configuraciones (`EntityConfigEditor`), Centro de Atención (`AttentionSheet` y badges en tiempo real) y pantalla completa de Ajustes y Credenciales (`SettingsScreen` y `ProviderCredentialsSheet`) con persistencia segura en `FlutterSecureStorage`.
...
- Desincronización de UI en Sesiones Largas (Plan 31): Limpieza activa de listeners de WebSocket en servidor (`wsRegistry.clearUnsub` / `setUnsub`), emisión de eventos no bloqueante mediante `Promise.allSettled` en `TypedEventEmitter`, des-suscripción limpia en cliente vía `useConnectionAwareEffect`, y resincronización reactiva del estado `streaming` del chat a través de eventos `session_status`.
...
- Unificación de Herramientas de Tareas (`task.tool`) y Memoria (`memory.tool`): Consolidación de `decompose_tasks`, `update_task_status` y `complete_task_list` en una sola herramienta `task` (acciones `start`, `update`, `end`, `status`); consolidación de `memory_store`, `memory_recall` y `memory_forget` en una sola herramienta `memory` (acciones `read`, `upsert`, `delete`); migración transparente de nombres legacy en metadatos de sesión en disco (`metadata.json`); y eliminación completa de la herramienta inoperativa `manage_pipelines`.
...
- Extensibilidad de Arquitectura (Plan 09 & Simplificación): Abstracción de Servicios (`ISessionStore`, `IArtifactStore`, `IMemoryStore` con implementaciones `File*Store` y `Memory*Store`), Abstracción de Herramientas (`BaseTool`, `FunctionTool`, `ToolRegistry` con namespaces y `legacyToolToBaseTool` adapter), Abstracción de Proveedores de Modelo (`BaseLlmProvider`, `LLMRegistry` y `ModelEnrichmentService`), Consolidación de Ajustes en Configuración (`memoryEnabled`, `memoryAutoStore` y `exaSearchEnabled` en Settings > General, con eliminación del código durmiente de plugins y de `PluginsPage`), y Empaquetado SDK interno `packages/spaces-sdk`.
...
- Modularización de Herramientas del Sistema (`ITool`) (Plan 19 / Hito 3): Encapsulamiento de las herramientas nativas (`read`, `write`, `edit`, `grep`, `find`, `ls`, `bash`) en clases independientes que implementan la interfaz `ITool`, ejecutables mediante `IToolExecutor` sin acoplamiento a módulos vendor y verificadas con suite de pruebas unitarias (`system-tools.test.ts`).
- Persistencia Limpia `FilesystemSessionStore` (Plan 20 / Hito 4): Implementación desacoplada y atómica de la interfaz `ISessionStore` (`FilesystemSessionStore` / `FileSessionStore`), separando el almacenamiento en disco JSONL (`messages.jsonl`) y metadatos (`metadata.json`) de las clases de IA, con escrituras seguras bajo concurrencia, paginación, filtros de consulta, adaptador `SessionStoreAdapter` e inyección por interfaz en `AgentRuntime` y `SpacesHost`.
- Proveedor de Modelo Unificado `OpenAICompatibleProvider` (Plan 21 / Hito 5): Implementación desacoplada de `IModelProvider` basada en `fetch` nativo de Bun/Node para cualquier endpoint compatible con OpenAI `/v1/chat/completions`, con streaming SSE en tiempo real, ensamblado de `tool_calls` fragmentadas, deltas de razonamiento (`reasoning_content`), recolección de métricas de tokens de uso, adaptador `ModelProviderAdapter` e inyección opcional en `AgentRuntime`.
- Eliminación de Duplicaciones y Componentes Reutilizables en Frontend (Plan 24 / Hito 3): Abstracción unificada del sistema de modales con `Dialog` y `FormDialog`, migración de los 9 modales de la aplicación; consolidación del EventBus centralizado `EntityEventBus` para eventos `entity-updated` eliminando 30 dispatches inline; servicio de `localStorage` fuertemente tipado (`storage` + `useLocalStorage`); primitivas de formulario `FormField` y `FormSection`; y hooks de teclado/eventos `useEscapeKey` y `useClickOutside`.
- Motor de Workflows de Alta Robustez: Nodos de control de flujo (`if`, `switch`, `merge`) con podado dinámico de ramas; motor de expresiones JSONata con scope `$inputs`, `$steps`, `$run` e interpolación de plantillas; nodos de aprobación humana (`approval`) pausados con notificación en Attention Hub y eventos WebSocket; testing determinista con Data Pinning y modo `dryRun`; y ejecución de código JS sin LLM en V8 Isolate aislado (`isolated-vm`).
- Herramienta `manage_workflow` (Single Tool Pattern) para el Agente Global: Integración de la herramienta unificada `manage_workflow` con contrato declarativo formal de esquemas (`action: "contract"` disponible siempre), operaciones CRUD completas (`list`, `get`, `save`, `delete`), manipulación granular de nodos y conexiones (`add_step`, `update_step`, `remove_step`, `connect_steps`), control de ciclo de vida de ejecuciones (`run`, `get_run`, `list_runs`, `abort`, `approve`), catálogo SSOT en `tools-catalog.ts` y guía de arquitectura y debugging `.agents/rules/workflow.rules.md`.
...
- Catálogo y configuración de 9 proveedores de IA: OpenAI, Google Gemini, xAI/Grok, DeepSeek, Groq, Mistral, OpenRouter, Qwen y OpenCodeGo, con selección dinámica de proveedor por omisión (`defaultProvider`) y modelo por omisión por proveedor (`providerDefaults`).
...
- Aplicación Móvil Spaces (`apps/mobile`): Cliente nativo multiplataforma en Flutter (Dart 3) y Riverpod, con arquitectura desacoplada por features (Auth, Dashboard, Sessions, Chat en streaming con rendering Markdown, Projects & Agents, Attention Hub, Settings & Config, Teams & Workflows), navegación reactiva mediante GoRouter, persistencia segura de credenciales en FlutterSecureStorage, ErrorBoundary global con retry, banner offline reactivo (`connectivity_plus`), Skeleton Loaders con animación shimmer en todas las vistas, dark mode completo con tokens de diseño unificados, CI/CD en GitHub Actions y suite completa de tests automatizados (196 pruebas en verde con `flutter analyze` en 0 issues).
