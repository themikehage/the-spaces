...
export function DelegationsPanel({
  ...
  const handleAbort = useCallback(
    async (toolCallId: string, e?: React.MouseEvent) => {
  ...
  return (
    <div className="h-full flex flex-col md:flex-row min-w-0 bg-bg overflow-hidden">
      {/* List Area */}
      <div
        className={`flex-1 flex flex-col min-w-0 h-full ${selectedDelegationId ? "hidden md:flex" : "flex"}`}
      >
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <span
                        className={`text-[10px] font-mono px-2 py-0.5 rounded border ${statusBg} flex items-center gap-1.5 font-medium`}
                      >
                        <span
                          className={`w-1.5 h-1.5 rounded-full ${statusColor} ${isRunning ? "animate-pulse" : ""}`}
                        />
                        {statusText}
                      </span>
                    </div>
          ...
          >
            <div className="p-4 border-b border-border bg-surface/50 flex items-center justify-between">
              <div>
                ...
                <p className="text-[10px] text-text-secondary font-mono truncate max-w-[200px]">
                  ID: {selectedDelegation.toolCallId}
                </p>
              </div>
              <button
              ...
              <div className="grid grid-cols-2 gap-3 text-xs">
                ...
                <div className="p-2.5 bg-surface/30 border border-border/30 rounded-lg flex flex-col gap-0.5">
                  ...
                  <span className="font-mono text-text-primary text-[10px]">
                    {selectedDelegation.startedAt
                      ? new Date(selectedDelegation.startedAt).toLocaleTimeString()
                      : "-"}
                  </span>
                </div>
              </div>
            ...
            <div className="p-4 border-t border-border bg-surface/50 flex flex-col gap-2">
              {selectedDelegation.status === "running" && (
                ...
                >
                  {abortingCallId === selectedDelegation.toolCallId
                    ? "Cancelando..."
                    : "Abortar Delegación"}
                </button>
              )}
