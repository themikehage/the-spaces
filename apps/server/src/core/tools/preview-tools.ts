...
import { abortBuild, runBuild } from "../preview-builder";
  ...
  return [
    {
      name: "manage_preview",
      description:
        "Manage the project's build preview configuration and execution state (get status, configure build settings, run build, or abort build).",
      parameters: {
        type: "object",
        properties: {
          action: {
            ...
            description:
              "The action to perform: 'status' to get current settings and state, 'configure' to update settings, 'build' to trigger a build, or 'abort' to stop an active build.",
          },
          config: {
            type: "object",
            description:
              "The preview configuration parameters. Required only when action is 'configure'.",
            properties: {
              framework: {
                type: "string",
                enum: ["auto", "vite", "next", "nuxt", "astro", "html", "custom"],
                description:
                  "The framework preset to use. Use 'html' for static websites without a build step.",
              },
              buildCommand: {
                type: "string",
                description:
                  "Custom build command to compile the app (e.g. 'npm run build', 'vite build').",
              },
              outputDir: {
                type: "string",
                description:
                  "The directory where production build assets are generated (e.g. 'dist', 'build', '.next').",
              },
            },
            required: ["framework"],
          },
            ...
            case "configure": {
              if (!args.config) {
                return {
                  content: [
                    {
                      type: "text",
                      text: "Error: 'config' parameter is required when action is 'configure'",
                    },
                  ],
              ...
              return {
                content: [
                  {
                    type: "text",
                    text: `Preview configuration updated: ${JSON.stringify(saved, null, 2)}`,
                  },
                ],
