...
export async function runImageGenModel(
  ...
  if (isQwen) {
    const apiKey =
      ...
      "";
    console.log(
      `[QWEN IMAGE GEN] Resolving apiKey. Length: ${apiKey.length}. Start: ${apiKey.substring(0, 8)}... End: ${apiKey.substring(apiKey.length - 8)}`,
    );
    if (!apiKey) {
      throw new Error(
    ...
    if (!res || !res.ok) {
      throw new Error(
        lastErrorMsg || "Failed to execute image generation call on DashScope endpoints.",
      );
    }

    const data = await res.json();
    const imgUrl =
      data.output?.choices?.[0]?.message?.content?.[0]?.image || data.output?.results?.[0]?.url;
  ...
  console.log(
    `[OPENROUTER IMAGE GEN] Resolving apiKey. Length: ${apiKey?.length || 0}. Start: ${apiKey?.substring(0, 8)}...`,
  );
  if (!apiKey) {
    throw new Error(
  ...
  if (!modelObj) {
    throw new Error(
      `Image generation model '${modelId}' is not configured or available. Please verify your OpenRouter credentials.`,
    );
  }
...
export function createImageGenTool(workspaceDir: string, username: string) {
  return {
    name: "generate_image",
    description:
      "Generate an image from a text prompt using an AI image generation model configured in settings.",
    parameters: {
      type: "object",
      properties: {
        prompt: {
      ...
      if (!modelId) {
        return {
          content: [
            {
              type: "text",
              text: "Error: No image generation model configured. Please configure an image generation model in Settings > General Tab before calling generate_image.",
            },
          ],
          isError: true,
        };
      ...
      try {
        ...
        return {
          content: [
            {
              type: "text",
              text: `Successfully generated image and saved to workspace: ${localPath}`,
            },
          ],
          details: {
            status: "success",
        ...
        return {
          content: [
            {
              type: "text",
              text: `Error executing image generation tool: ${err.message || String(err)}`,
            },
          ],
          isError: true,
        };
