...
import type { JsonlSessionStore } from "../ai/session-persistence";
import type { Agent } from "../ai/vendor/agent/src/agent.ts";
...
export class NavigationController {
  ...
  async abort(agent?: Agent, abortController?: AbortController | null): Promise<void> {
    ...
    try {
      const registry =
        this.delegationRegistry ?? (await import("./delegation-registry")).delegationRegistry;
