...
import { getEnvironmentContext } from "../infra/env-check";
...
export function buildSubagentInstructions(task: string, role?: string): string {
  return [
    ...
    `3. Your final message MUST contain the structured result envelope below in JSON format:`,
    `\`\`\`json`,
    `{`,
...
export function wrapDelegationTask(task: string, targetType: string): string {
  return [
    ...
    "After completing the task above, your FINAL message MUST contain this result envelope in JSON format:",
    "```json",
    "{",
    '  "status": "success" | "partial" | "blocked",',
    '  "executive_summary": "<1-3 sentences summarizing what was accomplished>",',
